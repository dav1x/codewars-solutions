...
func main() {
	...
	bondMap := make(map[string][]string)

	bondMap["bond_james"] = []string{"shaken, not stirred", "martinis", "fast cars"}
	bondMap["moneypenny_jenny"] = []string{`intelligence`, `literature`, `computer science`}
	bondMap["no_dr"] = []string{`cats`, `ice cream`, `sunsets`}
	bondMap["phillips_davis"] = []string{"tacos"}
	bondMap["fleming_ian"] = []string{`steaks`, `cigars`, `espionage`}
	// value semantics
	//a := 1
	//fmt.Printf("%#v\n", bondMap)
	//fmt.Println(bondMap) // 1
	//fmt.Print(bondMap["bond_james"])
	delete(bondMap, "phillips_davis")

	for k, v := range bondMap {
		fmt.Println(k)
		for i, v2 := range v {
			fmt.Println("I love", i, v2)
		}
	}
